...
class MongooseService {
    ...
    filter = [
        ...
        { $project: { date: 1, item: 1, total: { $multiply: ["$vendor_products.price", "$quantity"] } } }
        // {
        //     $addFields: {
        ...
        return new Promise((resolve, reject) => {
            ...
            model.aggregate(
                [
                    ...
                    // { $project: { 'vendor_product_id.price':1,  user_id: 1, quantity:1,  total: { $multiply: [ 10, "$quantity" ] } } }
                    { $project: { vendor_product_id: 1, user_id: 1, quantity: 1, total_price: { $multiply: [{ $arrayElemAt: ["$vendor_product_id.price", 0] }, "$quantity"] } } }
